...
1. Create build images containing the build tool-chain, Java and an Apache Cassandra git working directory
   * Debian Bookworm with JDK 17:
   ```docker build -t cass-build-debs -f docker/bookworm-image.docker docker/```
   * RPM:
...
2. Run build script through docker (specify branch, e.g. cassandra-3.0 and version, e.g. 3.0.11):
   * Debian Bookworm
    ```docker run --rm -v `pwd`/dist:/dist `docker images -f label=org.cassandra.buildenv=bookworm -q` /home/build/build-debs.sh <branch/tag>```
   * RPM:
...
The Bookworm image provides JDK 17. Pass `17` to `build-debs.sh` when using it:

```docker run --rm -v `pwd`/dist:/dist `docker images -f label=org.cassandra.buildenv=bookworm -q` /home/build/build-debs.sh <branch/tag> 17```
